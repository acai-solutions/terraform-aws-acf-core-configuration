...
def flatten_map(input_map, parent_key='', separator='/', encode_values=False, safe_chars=""):
    """
    ...
    if isinstance(input_map, dict):
        for key, value in input_map.items():
            ...
            items.extend(flatten_map(
                value, new_key, separator=separator, 
                encode_values=encode_values, safe_chars=safe_chars
            ).items())
    elif isinstance(input_map, list):
        for idx, item in enumerate(input_map):
            new_key = f"{parent_key}{separator}_{idx}_"
            items.extend(flatten_map(
                item, new_key, separator=separator,
                encode_values=encode_values, safe_chars=safe_chars
            ).items())
    else:
        # Apply encoding if requested
        final_value = encode_value(input_map, safe_chars) if encode_values else input_map
