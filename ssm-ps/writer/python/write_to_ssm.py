...
def main():
    ...
    for k, v in param_map.items():
        try:
            ...
            if (
                ...
                print(
                    f"Error: Parameter {k} exists but is managed by a different cluster ID ({existing_cluster_id}). Current cluster ID is {cluster_id}."
                )
                sys.exit(1)
                    ...
                    if tags:
                        ...
                        if old_tag_keys:
                            ssm.remove_tags_from_resource(
                                ...
                                ResourceId=k,
                                TagKeys=old_tag_keys,
                            )

                        ssm.add_tags_to_resource(
                            ResourceType="Parameter", ResourceId=k, Tags=tags
                        )
    ...
    try:
            ...
            for param in page["Parameters"]:
                ...
                try:
                    ...
                    if e.response["Error"]["Code"] == "ParameterNotFound":
                        ...
                        print(
                            f"Parameter {param_name} was deleted during processing, skipping..."
                        )
                        continue
                    else:
        ...
        if not obsolete_params:
            ...
            print(
                f"Found {len(obsolete_params)} obsolete parameters to delete: {obsolete_params}"
            )
